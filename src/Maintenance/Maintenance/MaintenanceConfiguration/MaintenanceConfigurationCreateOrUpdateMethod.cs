...
namespace Microsoft.Azure.Commands.Maintenance
{
    ...
    public partial class NewAzureRmMaintenanceConfiguration : MaintenanceAutomationBaseCmdlet
    {
        ...
        [Parameter(
            Mandatory = false,
            HelpMessage = "List of KBs")]
        public HashSet<string> WindowParameterKbNumberToInclude { get; set; }

        [Parameter(
            Mandatory = false,
            HelpMessage = "List of KBs excluded during patching")]
        public HashSet<string> WindowParameterKbNumberToExclude { get; set; }

        [Parameter(
            Mandatory = false,
            HelpMessage = "Windows Patch classification.")]
        public HashSet<string> WindowParameterClassificationToInclude { get; set; }

        [Parameter(
            Mandatory = false,
            HelpMessage = "Exclude KBs required reboot")]
        public bool? WindowParameterExcludeKbRequiringReboot { get; set; }

        [Parameter(
            Mandatory = false,
            HelpMessage = "List of task executed before schedule. e.g. [{'source' :'runbook', 'taskScope': 'Global', 'parameters': { 'arg1': 'value1'}}]")]
        public string PreTask { get; set; }

        [Parameter(
            Mandatory = false,
            HelpMessage = "List of task executed after schedule. [{'source' :'runbook', 'taskScope': 'Resource', 'parameters': { 'arg1': 'value1'}}]")]
        public string PostTask { get; set; }

        [Parameter(
            Mandatory = false,
            HelpMessage = "List of installPatches.")]
